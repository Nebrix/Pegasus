    ...
    while (running) {
        ...
        if (strcmp(tokens[0], "exit") == 0) {
            ...
            if (tokenCount > 2) {
                ...
                char command[MAX_INPUT_SIZE];
                snprintf(command, sizeof(command), "python3 src/tools/dirb/dirb.py %s", tokens[1]);
                int result = system(command);
                if (result == -1) {
                    perror("system");
                }
            ...
            char command[MAX_INPUT_SIZE];
            snprintf(command, sizeof(command), "python3 src/tools/packet-sniffer/packet-sniffer.py");
            int result = system(command);
            if (result == -1) {
                perror("system");
            }
